...
function deriveAncestorStatusFromChildren(
  ...
  const doneCount = relevantChildren.filter((child) => child.status === "done").length;
  if (totalCount === 0) {
    return { nextStatus: null, closeoutPending: false, doneCount, totalCount };
  }

  const allDone = doneCount === totalCount;
  if (allDone) {
    return {
      nextStatus: parentStatus === "done" ? "done" : "in_progress",
      closeoutPending: parentStatus !== "done",
      doneCount,
      totalCount,
    };
  }
  if (relevantChildren.some((child) => ACTIVE_CHILD_ISSUE_STATUSES.has(child.status))) {
    return { nextStatus: "in_progress", closeoutPending: false, doneCount, totalCount };
  }
  if (doneCount > 0) {
    return { nextStatus: "in_progress", closeoutPending: false, doneCount, totalCount };
  }
  if (relevantChildren.some((child) => child.status === "blocked")) {
    return { nextStatus: "blocked", closeoutPending: false, doneCount, totalCount };
  }
  if (relevantChildren.every((child) => child.status === "backlog")) {
    return { nextStatus: "backlog", closeoutPending: false, doneCount, totalCount };
  }
  return { nextStatus: "todo", closeoutPending: false, doneCount, totalCount };
}
...
async function recomputeAncestorStatuses(
  ...
  while (queue.length > 0) {
    ...
    const {
      ...
      doneCount,
      totalCount,
    } = deriveAncestorStatusFromChildren(children, parent.status);
    let wroteStatusUpdate = false;
    ...
    // Keep parent issues open until the assignee explicitly closes them,
    // even after every child lane has reached done.
    if (nextStatus && (wroteStatusUpdate || closeoutPending)) {
      const statusEmoji: Record<string, string> = {
        done: "✅", in_progress: "🔄", blocked: "🚫", todo: "📋", backlog: "📥",
      };
      const statusComment = closeoutPending
        ? `✅ **All child lanes complete; explicit parent closeout required** (${doneCount}/${totalCount} lanes done)\n\n> Paperclip keeps this parent open until the assignee explicitly marks it done.`
        : `${statusEmoji[nextStatus] ?? "📌"} **Batch status updated → \`${nextStatus}\`** (${doneCount}/${totalCount} lanes done)\n\n> Auto-generated by Paperclip on child lane status change.`;
      await dbOrTx
        ...
        .catch(() => {/* non-fatal: comment is informational only */});
    }
...
export function issueService(db: Db) {
  ...
  async function repairExplicitStatusTruth(input: {
    ...
    const { issue, status, source, actorType, actorId, agentId, runId } = input;
    const summary = await statusTruth.getIssueStatusTruthSummary(issue.id);
    if (
      summary &&
      summary.authoritativeStatus === status &&
    ...
    const previousStatus =
      summary?.authoritativeStatus && summary.authoritativeStatus !== status
        ? summary.authoritativeStatus
        : summary?.persistedStatus && summary.persistedStatus !== status
          ? summary.persistedStatus
          : null;
    ...
    await logActivity(db, {
      ...
      details: {
        ...
        status,
        source,
        _previous: previousStatus ? { status: previousStatus } : undefined,
      },
    });
  }

  async function repairCheckoutStatusTruth(
    issue: Pick<typeof issues.$inferSelect, "id" | "companyId" | "identifier">,
    agentId: string,
    runId: string | null,
  ) {
    await repairExplicitStatusTruth({
      issue,
      status: "in_progress",
      source: "checkout",
      actorType: "agent",
  ...
  async function repairReleaseStatusTruth(
    issue: Pick<typeof issues.$inferSelect, "id" | "companyId" | "identifier">,
    input?: {
      actorType?: "agent" | "system";
      actorId?: string | null;
    ...
    await repairExplicitStatusTruth({
      ...
      actorId: input?.actorId ?? "paperclip",
  ...
  return {
    ...
    checkout: async (id: string, agentId: string, expectedStatuses: string[], checkoutRunId: string | null) => {
      ...
      if (updated) {
        await repairCheckoutStatusTruth(updated, agentId, checkoutRunId);
      ...
      if (
        ...
        if (adopted) {
          ...
          await repairCheckoutStatusTruth(row, agentId, checkoutRunId);
          const [enriched] = await withIssueLabels(db, [row]);
          return enriched;
        }
      }
    ...
    release: async (
      ...
      const updated = await db
        ...
        .then((rows) => rows[0] ?? null);
      if (!updated) return null;
      await repairReleaseStatusTruth(updated, {
        actorType: activityActor?.actorType ?? (actorAgentId ? "agent" : "system"),
        actorId: activityActor?.actorId ?? actorAgentId ?? "paperclip",
