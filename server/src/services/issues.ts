...
async function recomputeAncestorStatuses(
  ...
  while (queue.length > 0) {
    ...
    if (
      ...
      // Fix #7: Write a summary comment to the parent when its aggregate
      // status changes, so managers see a live audit trail without
      // manually refreshing the batch document.
      const doneCount = children.filter((c: { status: string }) => c.status === "done").length;
      const totalCount = children.filter((c: { status: string }) => c.status !== "cancelled").length;
      const statusEmoji: Record<string, string> = {
        done: "✅", in_progress: "🔄", blocked: "🚫", todo: "📋", backlog: "📥",
      };
      const emoji = statusEmoji[nextStatus] ?? "📌";
      const statusComment = `${emoji} **Batch status updated → \`${nextStatus}\`** (${doneCount}/${totalCount} lanes done)\n\n> Auto-generated by Paperclip on child lane status change.`;
      await dbOrTx
